...
class GeventServing():
    ...
    def run(self):
        ...
        try:
            ...
            host, port = (listener[0], int(listener[1]))
        except:
            print(f"Invalid bind address {bind}")

        server = gevent.pywsgi.WSGIServer((host, port), self.application, spawn = workers, environ={'wsgi.multithread': True,'wsgi.multiprocess': True,})
